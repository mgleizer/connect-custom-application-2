...
import GetChannelDetails from './get-channel-details.ctp.graphql';
import QueryChannels from './query-channels.ctp.graphql';
...
export const useChannelsFetcher = ({ page, perPage, tableSorting }) => {
  const { data, error, loading } = useMcQuery(QueryChannels, {
    variables: {
      limit: perPage.value,
...
export const useChannelDetailsFetcher = (channelId) => {
  const { data, error, loading } = useMcQuery(GetChannelDetails, {
    variables: {
      channelId,
    },
    context: {
